...
export default {
  ...
  methods: {
    ...
    printQR: function () {
      this.$http
        .put(`http://${process.env.BACKEND_SERVER}/api/pet/make/qr`, { did: this.qId })
        .then((res) => {
    ...
    getNoQrDids: function () {
      this.$http
        .get(`http://${process.env.BACKEND_SERVER}/api/pet/noqr`)
        .then((res) => {
          if (res.data.rows) {
    ...
    makeQR: function (params) {
      this.text = `http://${process.env.FRONTEND_SERVER}/#/pet/own/${params.row.did}`;
